...
class Plugin {
  attach({Session session, Callbacks callbacks}) {
    createHandle(session, callbacks);
  }

  createHandle(Session session, Callbacks callbacks) {
    if (!session.connected) {
      Janus.warn("Is the server down? (connected=false)");
      session.gatewayCallbacks.error("Is the server down? (connected=false)");
      return;
    }
    ...
    String handleToken =
        callbacks.token != null ? callbacks.token : session.token; // FIXME
    ...
    if (session.websockets != null) {
      session.transactions[transaction] = (json) {
        Janus.debug(json);
        ...
        Map<String, dynamic> pluginHandle = {
          ...
          'getId': () => handleId,
          'getPlugin': () => plugin,
          'getVolume': () => getVolume(session, handleId, true),
          'getRemoteVolume': () => getVolume(session, handleId, true),
          'getLocalVolume': () => getVolume(session, handleId, false),
          'isAudioMuted': () => isMuted(session, handleId, false),
          'muteAudio': () => mute(session, handleId, false, true),
          'unmuteAudio': () => mute(session, handleId, false, false),
          'isVideoMuted': () => isMuted(session, handleId, true),
          'muteVideo': () => mute(session, handleId, true, true),
          'unmuteVideo': () => mute(session, handleId, true, false),
          'getBitrate': () => getBitrate(session, handleId),
          'send': (callbacks) => sendMessage(session, handleId, callbacks),
          'data': (callbacks) => sendData(session, handleId, callbacks),
          'dtmf': (callbacks) => sendDtmf(session, handleId, callbacks),
          ...
          'createOffer': (callbacks) =>
              prepareWebrtc(session, handleId, true, callbacks),
          'createAnswer': (callbacks) =>
              prepareWebrtc(session, handleId, false, callbacks),
          'handleRemoteJsep': (callbacks) =>
              prepareWebrtcPeer(session, handleId, callbacks),
          ...
          'hangup': (sendRequest) =>
              cleanupWebrtc(session, handleId, sendRequest == true),
          'detach': (callbacks) => destroyHandle(session, handleId, callbacks)
        };

        session.pluginHandles[handleId] = pluginHandle;
        callbacks.success(pluginHandle);
      };

      request["session_id"] = session.sessionId;
      session.ws.send(jsonEncode(request));
      return;
    }
  ...
  sendMessage(Session session, String handleId, Callbacks callbacks) {
    if (!session.connected) {
      Janus.warn("Is the server down? (connected=false)");
      callbacks.error("Is the server down? (connected=false)");
      return;
    }
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    ...
    Janus.httpAPICall(
        session.server + "/" + session.sessionId + "/" + handleId,
        {
          'verb': 'POST',
          'withCredentials': session.withCredentials,
          'body': request
  ...
  sendTrickleCandidate(Session session, String handleId, candidate) {
    if (!session.connected) {
      Janus.warn("Is the server down? (connected=false)");
      return;
    }
  ...
  createDataChannel(
      Session session, String handleId, dclabel, incoming, pendingData) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      return;
  ...
  sendData(Session session, String handleId, Callbacks callbacks) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      callbacks.error("Invalid handle");
    ...
    if (!config['dataChannel'][label]) {
      ...
      createDataChannel(session, handleId, label, false, data);
      callbacks.success();
      return;
    }
  ...
  sendDtmf(Session session, String handleId, Callbacks callbacks) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      callbacks.error("Invalid handle");
  ...
  destroyHandle(Session session, String handleId, Callbacks callbacks) {
    ...
    Janus.log("Destroying handle " +
        ...
        noRequest.toString() +
        ")");
    cleanupWebrtc(session, handleId, false);
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      // Plugin was already detached by Janus, calling detach again will return a handle not found error, so just exit here
      session.pluginHandles.remove(handleId);
      callbacks.success();
      return;
    }
    if (noRequest) {
    ...
    if (session.websockets != null) {
      request["session_id"] = session.sessionId;
      request["handle_id"] = handleId;
      session.ws.send(jsonEncode(request)); // FIX ME
      session.pluginHandles.remove(handleId);
      callbacks.success();
      return;
    }
  ...
  // WebRTC stuff
  streamsDone(Session session, String handleId, jsep, Map media, callbacks,
      MediaStream stream) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
    ...
    if (config['pc'] == null) {
      Map<String, dynamic> pc_config = {
        ...
        "bundlePolicy": session.bundlePolicy
      };
      ...
      config['pc'].onIceCandidate = (event) {
        if (event.candidate == null ||
          ...
          if (config['trickle'] == true) {
            ...
            sendTrickleCandidate(session, handleId, {"completed": true});
          } else {
            // No trickle, time to send the complete SDP (including all candidates)
            sendSDP(session, handleId, callbacks);
          }
        } else {
          ...
          if (config['trickle'] == true) {
            // Send candidate
            sendTrickleCandidate(session, handleId, {});
          }
        }
      };
      ...
      tracks.forEach((MediaStreamTrack track) {
        ...
        if (!simulcast2) {
          ...
          if (track.kind == "audio") {
            ...
            Janus.log('Enabling rid-based simulcasting:' + track.toString());
    ...
    if (isDataEnabled(media) &&
      ...
      createDataChannel(
          session, handleId, Janus.dataChanDefaultLabel, false, null);
      config['pc'].ondatachannel = (event) {
        Janus.log("Data channel created by Janus:" + event); // FIX ME
        createDataChannel(
            session, handleId, event.channel.label, event.channel, null);
      };
    }
    ...
    if (!jsep) {
      createOffer(session, handleId, media, callbacks, null);
    } else {
      config['pc'].setRemoteDescription(jsep).then(() {
        ...
        createAnswer(session, handleId, media, callbacks, null);
      }, callbacks.error);
    }
  }

  prepareWebrtc(session, handleId, offer, callbacks) {
    var jsep = callbacks.jsep;
    if (offer && jsep) {
    ...
    if (config['pc'] == null) {
      ...
      if ((isAudioSendEnabled(media) && media['keepAudio']) &&
        ...
        streamsDone(
            session, handleId, jsep, media, callbacks, config['myStream']);
        return;
      }
    }
    ...
    if (callbacks.stream) {
      ...
      pluginHandle['consentDialog'](false);
      streamsDone(session, handleId, jsep, media, callbacks, stream);
      return;
    }
    if (isAudioSendEnabled(media) || isVideoSendEnabled(media)) {
            ...
            navigator.getDisplayMedia(constraints).then((MediaStream stream) {
              ...
              if (isAudioSendEnabled(media) && !media['keepAudio']) {
                navigator.getUserMedia({'audio': true, 'video': false}).then(
                  ...
                  streamsDone(
                      session, handleId, jsep, media, callbacks, stream);
                });
              } else {
                streamsDone(session, handleId, jsep, media, callbacks, stream);
              }
      ...
      if (media == null || media['video'] != 'screen') {
        ...
        navigator.getSources().then((sources) {
          ...
          if (!gumConstraints['audio'] && !gumConstraints['video']) {
            pluginHandle['consentDialog'](false);
            streamsDone(
                session, handleId, jsep, media, callbacks, callbacks.stream);
          } else {
      ...
      streamsDone(session, handleId, jsep, media, callbacks, null);
    }
  }
  ...
  createOffer(session, handleId, media, callbacks, customizeSdp) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      callbacks.error("Invalid handle");
  ...
  createAnswer(session, handleId, media, callbacks, customizedSdp) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      callbacks.error("Invalid handle");
  ...
  sendSDP(session, handleId, callbacks) {
    var pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
  ...
  getVolume(session, handleId, remote) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      return 0;
  ...
  isMuted(session, handleId, video) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      return true;
  ...
  mute(session, handleId, video, mute) {
    Map<String, dynamic> pluginHandle = session.pluginHandles[handleId];
    if (pluginHandle == null || pluginHandle['webrtcStuff'] == null) {
      Janus.warn("Invalid handle");
      return false;
  ...
  cleanupWebrtc(session, handleId, hangupRequest) {
    ...
    if (config != null) {
      if (hangupRequest == true) {
        ...
        if (pluginHandle['token']) request["token"] = pluginHandle['token'];
        if (session.apiSecret) request["apisecret"] = session.apiSecret;
